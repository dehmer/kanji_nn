...
def split_into_strokes(raw):
    ...
    status_dropped = raw[:, [0, 1, 3]]
    return np.split(status_dropped, split_indices[:-1])
...
def metrics(stroke):
    # xs, ys, ts = stroke[:, 0], stroke[:, 1], stroke[:, 2]                 # n
    diffs = np.diff(stroke, axis=0)
    dx, dy, dt = diffs[:, 0], diffs[:, 1], diffs[:, 2]                    # n - 1
    print(dx)
    print()

    # 2. Compute metrics
    distances = np.sqrt(dx**2 + dy**2)
    velocities = distances                                               # n - 1
    ...
    angles = np.degrees(np.arccos(dot_products))                          # n - 2
    ...
    stacked = np.vstack((stroke[:, 2], velocities, angles)).T

    # low velocity, high angle
    candidates = np.where((stacked[:, 1] < 10) & (stacked[:, 2] > 60))
    print('[candidates]', candidates)
    for index in candidates:
        print('ts', stroke[index, 2])

    plot_dynamics(stroke[:, 2], velocities, angles)
