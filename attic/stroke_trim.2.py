...
class TrimConfig:
    ...
    selfintersect_min_gap_frac: float = 0.25  # a close non-adjacent pair only counts as a candidate loop if
                                               # at least this fraction of the stroke's total arc length was
                                               # traveled between them -- excludes stationary pauses/dwells,
                                               # which sit close in space without the path having gone anywhere
...
def _min_dist_excluding_dwell(x, y, arclen_cum, min_gap_arclen):
    """
    Like Step 1's min_dist_to_nonadjacent, but a candidate pair only
    counts if the path traveled at least `min_gap_arclen` of arc length
    between the two samples. This is what actually distinguishes a
    genuine loop/closure (comes back close after going somewhere) from
    a stationary pause (many samples sit near each other simply because
    the pen didn't move, regardless of how many indices apart they are).
    """
    n = len(x)
    out = np.full(n, np.inf)
    for i in range(n):
        mask = np.abs(arclen_cum - arclen_cum[i]) >= min_gap_arclen
        if not mask.any():
            continue
...
def _check_guards(x, y, t, arclen_cum, config: TrimConfig, body_mask):
    n = len(x)
    ...
    # outside the start/end transient windows -- AND when it happens
    # after real travel, not during a stationary pause (see
    # _min_dist_excluding_dwell above).
    ...
    if candidate_mask.any():
        min_gap = config.selfintersect_min_gap_frac * total_arclen
        dwell_aware_dist = _min_dist_excluding_dwell(x, y, arclen_cum, min_gap)
        interior = dwell_aware_dist[candidate_mask]
...
def derive_trim(
    ...
    status, detail = _check_guards(x, y, t, arclen_cum, config, body_mask)
